...
RSpec.describe Bible::Versions::Asvd do
  ...
  def stub_search_type search_type, matches
    ...
    allow(@asvd).to receive(:encode).with(matches[:full_match]).and_return(matches[:full_match])
  end
  ...
  describe "#search" do
    ...
    context 'when using a full reference' do
      ...
      context 'when no verse number is provided' do
        it 'should return all verses from the reference' do
          keyword = 'Genesis 2'
          stub_search_type :reference, full_match: keyword, book: 'Genesis', chapter: 2, number: nil
          genesis1 = @asvd.search(keyword)
      ...
      context 'when a range of verses are provided' do
        it 'should return all verses from the reference' do
          ...
          expect(genesis1[8][:verse_number]).to eq 10
        end
      end
    ...
    shared_examples_for "keyword search" do |keyword, search_context, expected_matches|
      #puts "-------------------------#{keyword.inspect}"
      context 'when using match' do
        it "should return all or zero matches within #{search_context} for #{keyword}" do
          stub_search_type :keyword_in_section, full_match: "#{search_context}:#{keyword}", search_context: search_context, search_keyword: keyword
    ...
    context 'when using a keyword' do
      ...
      context 'when searching the whole bible' do
        it_should_behave_like 'keyword search', METHUSELAH, 'whole', 7
        it_should_behave_like 'keyword search', GOD_IS_GOOD, 'whole', 7
        it_should_behave_like 'keyword search', JESUS_OR_JOSHUA, 'whole', 1168
        it_should_behave_like 'keyword search', MARANATHA, 'whole', 1
        it_should_behave_like 'keyword search', FALSE_PROPHET, 'whole', 0
        it_should_behave_like 'keyword search', MAHER_SHALAL_HASH_BAZ, 'whole', 2
      end

      context 'when searching the old testament' do
        it_should_behave_like 'keyword search', METHUSELAH, 'old_testament', 6
        it_should_behave_like 'keyword search', GOD_IS_GOOD, 'old_testament', 6
        it_should_behave_like 'keyword search', JESUS_OR_JOSHUA, 'old_testament', 234
        it_should_behave_like 'keyword search', MARANATHA, 'ot', 0
        it_should_behave_like 'keyword search', FALSE_PROPHET, 'old_testament', 0
        it_should_behave_like 'keyword search', MAHER_SHALAL_HASH_BAZ, 'old_testament', 2
      end

      context 'when searching the new testament' do
        it_should_behave_like 'keyword search', METHUSELAH, 'new_testament', 1
        it_should_behave_like 'keyword search', GOD_IS_GOOD, 'new_testament', 1
        it_should_behave_like 'keyword search', JESUS_OR_JOSHUA, 'new_testament', 934
        it_should_behave_like 'keyword search', MARANATHA, 'new_testament', 1
        it_should_behave_like 'keyword search', FALSE_PROPHET, 'new_testament', 0
        it_should_behave_like 'keyword search', MAHER_SHALAL_HASH_BAZ, 'new_testament', 0
      end
    end
  end
end
